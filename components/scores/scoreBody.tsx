...
export default function ScoreBody(props: scoreBodyType) {
  ...
  const steps = [
    ...
    },
    {
      ...
      content: "마우스를 통해서 이 패널을 제어해서 악보를 그릴 수 있습니다.",
    },
    {
      target: `.${controllerStyles.length}`,
      content: "이것을 통해서 음표의 길이를 제어할 수 있습니다.",
    },
    {
    ...
    },
    {
      ...
      content: "이 패널을 통해서 드럼 악기의 음표를 그릴 수 있습니다.",
    },
    {
      target: `.${controllerStyles.soundList}`,
      content: "다음과 같이 약어로 악기를 표현합니다.",
    },
    {
      target: `.${controllerStyles.soundList}`,
      content: "HH: high hat / CR: crash / RI: ride / SN: snare",
    },
    {
      target: `.${controllerStyles.soundList}`,
      content: "HT: high top / MT: middle top / FT: floor top / BA: base drum",
    },

    {
      target: `.${controllerStyles.soundList}`,
      content: "각각 악기는 토글 형태로, 선택됨과 선택되지 않음으로 나뉩니다.",
    },
    {
    ...
    {
      target: `.${controllerStyles.soundActions}`,
      content: "DEL 버튼을 통해서 마지막에 추가된 음표를 제거합니다.",
    },
    {
      target: `.${styles.scoreBody}`,
      content: "제어 패널을 사용하지 않고 키보드로도 악보를 그릴 수 있습니다.",
    },
    {
      target: `.${styles.scoreBody}`,
      content: "음표 길이 제어는 Q(늘리기)와 E(줄이기)로 할 수 있습니다.",
    },
    {
      target: `.${styles.scoreBody}`,
      content: "쉼표는 R 로 선택 및 해제할 수 있습니다.",
    },
    {
      target: `.${styles.scoreBody}`,
      content: "그리고 악기의 경우, 패널의 첫번째 줄은 순서대로 ASDF",
    },
    {
      target: `.${styles.scoreBody}`,
      content: "패널의 두번째 줄은 순서대로 ZXCV 를 나타냅니다.",
    },
    {
      target: `.${styles.scoreBody}`,
      content:
  ...
  const handleJoyrideCallback = (data: CallBackProps) => {
    ...
    if (finishedStatuses.includes(status)) {
      window.localStorage.setItem("TOUR-DONE", "true");
    }
  };

  useEffect(() => {
    if (!window.localStorage.getItem("TOUR-DONE")) {
      setRunTour(true);
    }
  }, []);
